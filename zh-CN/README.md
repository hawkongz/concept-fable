...
2026 年 4 月，Askell 在一次播客访谈中分享了一个小方法，却在中文 AI 社区引发了广泛传播：
...
Askell 的原版思路是一个绝妙的起点。**概念寓言 Skill** 在此基础上系统化扩展为 **8 步工作流**，并加入了防套路、保质量的机制：

| Askell 原版思路 | 本 Skill 的系统化扩展 |
|:---|:---|
| 不出现概念名 | Step 5：三段式结构，精确控制揭晓时机 |
| 让故事承载意义 | Step 4：因果链覆盖检查——写作前验证隐喻点映射到因果链的每一环；Step 5：情节的「因→果→困境」必须复现概念的运作机制 |
| — | Step 3：根据概念特征自动匹配故事类型（博弈→古代寓言 / 渐进→日常生活 / 涌现→自然或机械 / 表象vs实际→对话 / 工具方案型→前后对比式） |
| — | Step 3：根据概念情感色彩匹配叙事语气（告诫 / 反直觉 / 权衡 / 机制） |
| — | Step 7：7 条自检——角色自然度、隐喻准确性、故事独立性、揭晓时机、简洁度、具象化、术语闭环 |
| — | 反模式表：6 种常见失败模式及修正方法 |
...
- **📚 8 步结构化工作流**：理解概念 → 确认场景 → 选择类型与语气 → 验证隐喻映射 → 写故事 → 附释义 → 自检 → 输出
- **🎭 故事类型自动匹配**：根据概念的核心特征（博弈/渐进/涌现/反差/工具方案），自动选择最合适的寓言类型
- **🎯 因果链贴合检查**：不是只碰「主题」，而是确保故事情节的因果链条精准映射概念的运作机制
- **🔀 工具方案型对比叙事**：对于「为了解决某个问题而存在」的概念（如 Node.js、缓存），故事中同时展示「没有它」和「有它」两个阶段，让价值通过反差自然浮现
...
**Claude 会：**
1. 确认概念理解，写出因果链：线程A持有锁1等待锁2 → 线程B持有锁2等待锁1 → 循环等待 → 永久卡死
2. 选择故事类型与语气（两方博弈 → 古代寓言，告诫/警示语气）
3. 验证隐喻映射：因果链的每一环是否都有对应故事节拍？（持有并等待 ✓、循环等待 ✓、互不释放 ✓）
4. 创作一篇不出现「死锁」二字的寓言
5. 在故事结尾揭晓概念
6. 附上专业释义和故事元素对应表
7. 自检后输出

故事可能是：姐姐握着盐罐，弟弟攥着糖罐。姐姐的菜就差一撮糖出锅，弟弟的汤就差一勺盐调味。两人手里都有对方要的东西，却谁也不肯先松手——菜凉了，汤也凉了。
